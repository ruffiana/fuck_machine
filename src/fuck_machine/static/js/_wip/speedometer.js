...
function drawCruise(options, speed) {
    /* Draw colored cruise arc */

    // TODO: This is a bit of a hack
    // to deal with the start angle of the dial being negative 
    var end = (options.dial.end - options.dial.start) * (speed / 100) + options.dial.start
    ...
    drawArc(
        ...
        options.gauge.cruise.width,
        options.gauge.cruise.color,
        1.0
    )
}

function drawSpeedometer(options, speed) {
    /* Draw colored speedometer arc */

    // Determine color based on current speed
    var color = options.gauge.color
    if (speed > (100 * options.gauge.danger.start)) {
        color = options.gauge.danger.color
    }
    else if (speed > (100 * options.gauge.warning.start)) {
...
function drawSpeedDisplay(options, speed) {
    ...
    options.ctx.fillText(
        Math.round(speed * (options.dial.max / 100)),
        options.center.X,
        options.center.Y
    )
}
...
export function delayedDraw(speed, speedometer) {
...
export function draw() {
    ...
    */
    ...
    if (canvas !== null && canvas.getContext) {
        ...
        // Draw Cruise
        drawCruise(options, currentSpeedometer)

        // Draw Speedometer
        drawSpeedometer(options, currentSpeed)

        // // Draw numerical speed display
        drawSpeedDisplay(options, currentSpeed)
